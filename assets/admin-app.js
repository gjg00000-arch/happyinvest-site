    ...
    return fetch(API + path, opts)
      .then(function (r) {
        return r.text().then(function (text) {
          var j = {};
          if (!text && !r.ok) {
            j = { error: "응답 본문이 비어 있습니다. HTTP " + r.status };
          } else if (text) {
            try {
              j = JSON.parse(text);
            } catch (e) {
              j = {
                error:
                  ...
                  r.status +
                  " — CloudFront가 POST를 막았거나 API 경로가 잘못됐을 수 있습니다. 브라우저 Network 탭에서 요청 상태를 확인하세요.",
              };
            }
          }
      ...
      .catch(function (err) {
        return {
          ok: false,
          status: 0,
          j: { error: (err && err.message) || "네트워크 오류(오프라인·CORS·차단 등)" },
        };
      });
  }
  ...
  function trySession() {
    ...
    api("/api/admin/me", { method: "GET" }).then(function (x) {
      if (!x.ok) {
        ...
        var msg =
          (x.j && x.j.error) ||
          "로그인 직후 세션 확인(/api/admin/me)에 실패했습니다. HTTP " +
            x.status +
            " — CloudFront·JWT_SECRET 불일치·토큰 거부 가능성이 있습니다. Network 탭에서 /api/admin/me 응답을 확인하세요.";
        setErr(msg);
        return;
      }
      var who = document.getElementById("admin-who");
      if (who) who.textContent = (x.j && x.j.email) || "";
      showApp(true);
      goTab("dash");
    });
  }

  if (!window.__MAGIC_ADMIN_LOGIN_INLINE__) {
    ...
    if (loginForm) {
      loginForm.addEventListener("submit", function (ev) {
        ...
        api("/api/admin/login", {
          ...
          if (!x.j || !x.j.token) {
            setErr(
              "서버 응답에 token 이 없습니다. 프록시가 본문을 잘랐거나 API 오류일 수 있습니다. Network 탭에서 /api/admin/login 응답 본문을 확인하세요."
            );
            return;
          }
          setToken(x.j.token);
