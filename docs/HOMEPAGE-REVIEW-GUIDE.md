...
- **`#aw-usd-base`(정적 하나은행 패널)**: JS로 `#wire-route-picker`가 동작할 때(`html.js-wire-route-active`) **화면에서 숨겨** 동적 표와 이중 노출을 막는다. 인페이지 앵커는 `#wire-route-picker`로 맞춘다.

- **EUR · DE(Banking Circle)**: 미국 CFSB용과 유사하게 **국내/EU 은행 IBAN 입력 예시** 점선 카드가 동적 블록에 붙는다.

- **KEB Hana USD**: 안내 카피에 **영문 Beneficiary 우선**, 한글 전용 UI일 때는 금융사에 확인하라는 문장을 포함한다.
...
- 마크업에 `#aw-usd-base`(하나은행 정적 표)가 남아 있으나, **동적 송금 UI가 활성화되면** CSS로 **화면에서 숨김**해 동적 표와 이중 노출을 피한다. 무JS 접근 시에는 여전히 보일 수 있다.
...
- **2026-05 (외부 검토 반영·코드 정리)**: 전역 `https://happyinvests.com` → `https://magicindicatorglobal.com` 치환, `billing` 상단 **브랜드·법인 안내** 문구, `#wire-route-picker` 통화 변경 **토스트**, 레거시 `#aw-usd-base` **JS 활성 시 숨김**·앵커 통일, **EUR IBAN 점선 예시 카드**, KEB Hana USD **수취인명 가이드** 보강.
