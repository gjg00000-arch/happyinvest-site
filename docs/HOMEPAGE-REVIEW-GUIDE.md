...
LLM에게 “컨텍스트로 줄 설명만” 줄 때에는 **위 [공식 웹사이트](#공식-웹사이트-기준-url)** 한 줄과 섹션 3·8·10을 우선 붙여 넣고, 법 무결성은 반드시 `legal/` 원문 교차 검증을 명시하면 재현 가능한 리뷰가 된다. 고객에게 사이트 안내를 줄 때는 **`https://magicindicatorglobal.com/`** 를 명시적으로 적는 것을 권장한다.
