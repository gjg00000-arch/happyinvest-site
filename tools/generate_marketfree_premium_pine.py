...
entryMode = input.string("Conservative", "Entry Mode", options = ["Conservative", "Attack"], group = "Mode", tooltip = "진입은 한 봉 high>ML>low 롱(limit ML), high>MH>low 숏(limit MH). 여기 값은 알림·JSON entry_mode 라벨만 구분.")
...
reentryCooldownBars = input.int(6, "Re-entry Cooldown Bars After Stop", minval = 0, group = "Risk", tooltip = "고정 손절 StopLong·StopSell 체결 후에만 쿨다운. ProfitStop 익절·TrailingStop 은 제외하여 연속 매매 가능.")
...
f_jsonBuy() =>
    string msg = "MagicCore BUY ML(limit) " + entryMode + " " + syminfo.ticker + " " + timeframe.period + " ML=" + str.tostring(ML, format.mintick) + " (한봉 high>ML>low)"
    "{\"event\":\"magic_core_buy\",\"kind\":\"signal\",\"entry_kind\":\"BuyLong\",\"limit_ml\":" + str.tostring(ML, format.mintick) + ",\"entry_mode\":\"" + entryMode + "\",\"license_pack\":\"" + LICENSE_FIELD + "\",\"pos_state\":" + str.tostring(posState) + ",\"ticker\":\"" + syminfo.ticker + "\",\"exchange\":\"" + syminfo.prefix + "\",\"tf\":\"" + timeframe.period + "\",\"close\":" + str.tostring(close) + ",\"bar_time\":" + str.tostring(time) + ",\"text\":\"" + jsonEsc(msg) + "\"}"
f_jsonSell() =>
    string msg = "MagicCore SELL MH(limit) " + entryMode + " " + syminfo.ticker + " " + timeframe.period + " MH=" + str.tostring(MH, format.mintick) + " (한봉 high>MH>low)"
    "{\"event\":\"magic_core_sell\",\"kind\":\"signal\",\"entry_kind\":\"BuySell\",\"limit_mh\":" + str.tostring(MH, format.mintick) + ",\"entry_mode\":\"" + entryMode + "\",\"license_pack\":\"" + LICENSE_FIELD + "\",\"pos_state\":" + str.tostring(posState) + ",\"ticker\":\"" + syminfo.ticker + "\",\"exchange\":\"" + syminfo.prefix + "\",\"tf\":\"" + timeframe.period + "\",\"close\":" + str.tostring(close) + ",\"bar_time\":" + str.tostring(time) + ",\"text\":\"" + jsonEsc(msg) + "\"}"
f_jsonExit() =>
...
// 진입: 종가 cross 없음. 한 봉 high>ML>low 롱·high>MH>low 숏(TV는 해당 봉의 high·low로 관통 여부만 판단).
rangeStraddlesML = high > ML and ML > low
rangeStraddlesMH = high > MH and MH > low
entryBuyRaw = rangeStraddlesML
entrySellRaw = rangeStraddlesMH
...
if isLong
    strategy.exit("LX", from_entry = "L", limit = longTarget, stop = longProtectStop, comment_profit = "ProfitStop Long", comment_loss = longTrailActive ? "TrailingStop Long" : "StopLong")
if isShort
    strategy.exit("SX", from_entry = "S", limit = shortTarget, stop = shortProtectStop, comment_profit = "ProfitStop Short", comment_loss = shortTrailActive ? "TrailingStop Short" : "StopSell")
// 청산 직후 체결 코멘트(알림·쿨다운 판별)
tradeJustClosed = strategy.closedtrades > nz(strategy.closedtrades[1], 0)
lastExitRaw = tradeJustClosed and strategy.closedtrades > 0 ? strategy.closedtrades.exit_comment(strategy.closedtrades - 1) : na
lastExitComment = na(lastExitRaw) ? "" : lastExitRaw
// 재진입 쿨다운: 고정 손절(StopLong/StopSell)에만 적용 · TrailingStop·ProfitStop 는 쿨다운 없음(연속 진입 허용)
var int cooldownOpenBar = na
fixedStopCooldown = tradeJustClosed and strategy.closedtrades > 0 and (str.contains(lastExitComment, "StopLong") or str.contains(lastExitComment, "StopSell"))
if fixedStopCooldown
    cooldownOpenBar := bar_index + 1 + reentryCooldownBars
else if not na(cooldownOpenBar) and bar_index >= cooldownOpenBar
    cooldownOpenBar := na
cooldownActive = not na(cooldownOpenBar) and bar_index < cooldownOpenBar
// 진입·바이셀 알림은 posState=0 과 쿨다운 규칙만 적용(+1 롱 · 0 무포지 · -1 숏 이 시그널과 동기되게). 밴드 스트래들만 검사 — exit/stop 판별로 진입을 막지 않음.
entryGate = posState == 0 and not cooldownActive
magicBuy = entryBuyRaw and entryGate
magicSell = entrySellRaw and entryGate
...
if licenseOk and magicSell
    ...
    strategy.entry("S", strategy.short, qty = 1, limit = MH, comment = "BuySell")
else if licenseOk and magicBuy
    fixedLongStop := dynamicLongStop
    strategy.entry("L", strategy.long, qty = 1, limit = ML, comment = "BuyLong")
// 청산 표시·알림: 위와 동일 exit 코멘트
showTrailExit = tradeJustClosed and str.contains(lastExitComment, "TrailingStop")
showStopExit = tradeJustClosed and (str.contains(lastExitComment, "StopLong") or str.contains(lastExitComment, "StopSell"))
showProfitExit = tradeJustClosed and str.contains(lastExitComment, "ProfitStop")
...
rbT = plot(showRainbow ? T_raw : na, title = "RB T", color = lineColRb(cT, cBearT), linewidth = lineWRb, trackprice = false)
rb2m = plot(showRainbow ? T2m_val : na, title = "RB T2-", color = lineColRb(cT2m, cBear2m), linewidth = lineWRb, trackprice = false)
...
plotshape(showSignals and showProfitExit ? close : na, title = "ProfitStop", style = shape.xcross, location = location.absolute, size = size.small, color = color.yellow, text = "ProfitStop", textcolor = color.white)
plotshape(showSignals and showStopExit ? close : na, title = "StopLongStopSell", style = shape.xcross, location = location.absolute, size = size.small, color = color.orange, text = "Stop", textcolor = color.white)
plotshape(showSignals and showTrailExit ? close : na, title = "TrailingStop", style = shape.xcross, location = location.absolute, size = size.small, color = color.aqua, text = "TrailingStop", textcolor = color.white)
plot(baseR, "Applied BaseR (%)", display = display.status_line, trackprice = false)
plot(baseRAuto, "Auto BaseR (%)", display = display.status_line, trackprice = false)
plot(fineTune, "Applied Fine Tune (x)", display = display.status_line, trackprice = false)
plot(rPct, "Applied R (%)", display = display.status_line, trackprice = false)
...
if isChartLastForUi
    ...
    table.cell(info, 0, 1, "진입 트리거", text_color = color.silver, text_size = size.small)
    table.cell(info, 1, 1, "H>ML>L / H>MH>L", text_color = color.aqua, text_size = size.small)
