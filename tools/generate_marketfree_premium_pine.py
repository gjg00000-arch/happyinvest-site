...
f_jsonBuy() =>
    ...
    "{\"event\":\"magic_core_buy\",\"kind\":\"signal\",\"entry_kind\":\"straddle_ml\",\"limit_ml\":" + str.tostring(ML, format.mintick) + ",\"entry_mode\":\"" + entryMode + "\",\"license_pack\":\"" + LICENSE_FIELD + "\",\"pos_state\":" + str.tostring(posState) + ",\"ticker\":\"" + syminfo.ticker + "\",\"exchange\":\"" + syminfo.prefix + "\",\"tf\":\"" + timeframe.period + "\",\"close\":" + str.tostring(close) + ",\"bar_time\":" + str.tostring(time) + ",\"text\":\"" + jsonEsc(msg) + "\"}"
f_jsonSell() =>
    ...
    "{\"event\":\"magic_core_sell\",\"kind\":\"signal\",\"entry_kind\":\"straddle_mh\",\"limit_mh\":" + str.tostring(MH, format.mintick) + ",\"entry_mode\":\"" + entryMode + "\",\"license_pack\":\"" + LICENSE_FIELD + "\",\"pos_state\":" + str.tostring(posState) + ",\"ticker\":\"" + syminfo.ticker + "\",\"exchange\":\"" + syminfo.prefix + "\",\"tf\":\"" + timeframe.period + "\",\"close\":" + str.tostring(close) + ",\"bar_time\":" + str.tostring(time) + ",\"text\":\"" + jsonEsc(msg) + "\"}"
f_jsonExit() =>
...
if licenseOk and magicSell
    ...
    strategy.entry("S", strategy.short, qty = 1, limit = MH, comment = "MH-straddle Short")
